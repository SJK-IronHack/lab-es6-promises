...
async function makeBroccoli() {
  const currentStep1 = await obtainInstruction("broccoli", 0);
  document.querySelector("#broccoli").innerHTML += `<li>${currentStep1}</li>`;
  const currentStep2 = await obtainInstruction("broccoli", 1);
  document.querySelector("#broccoli").innerHTML += `<li>${currentStep2}</li>`;
  const currentStep3 = await obtainInstruction("broccoli", 2);
  document.querySelector("#broccoli").innerHTML += `<li>${currentStep3}</li>`;
  const currentStep4 = await obtainInstruction("broccoli", 3);
  document.querySelector("#broccoli").innerHTML += `<li>${currentStep4}</li>`;
  const currentStep5 = await obtainInstruction("broccoli", 4);
  document.querySelector("#broccoli").innerHTML += `<li>${currentStep5}</li>`;
  const currentStep6 = await obtainInstruction("broccoli", 5);
  document.querySelector("#broccoli").innerHTML += `<li>${currentStep6}</li>`;
  const currentStep7 = await obtainInstruction("broccoli", 6);
...
Promise.all(arrayOfPromises)
  .then(allSteps => {
    ...
    for (let index = 0; index < arrayOfPromises.length; index++) {
      document.querySelector("#brusselsSprouts").innerHTML += `<li>${allSteps[index]}</li>`;
    }
